...
function fmtWeekday(dt) {
 if (!dt) return "-";
 return new Intl.DateTimeFormat("en-IL", { weekday: "long" }).format(new Date(dt));
}
...
export default function RunDetails() {
 ...
 const ins = await supabase.from("children").insert([payload]).select("id").single();
 ...
 if (u.error) {
 setError(u.error);
 toast("Failed Refresh Enroll.", "danger");
 return;
 }
 toast(" Refresh Enroll.", "ok");
 await loadFixed();
 }
 ...
 <div style={{ minWidth: 0 }}>
 <div className="runLabel">Today / </div>
...
/>
 ...
 menuWidth="trigger"
 options={[
 { value: "balance_desc", label: ": ()" },
 { value: "balance_asc", label: ": ()" },
...
/>
 ...
 : <b>{Number(manageP.agreed_price || 0).toFixed(2)}</b> —
 Paid: <b>{Number(manageP.paid_amount || 0).toFixed(2)}</b> —
 ...
 <Modal
 open={openEnroll}
 title={enrollLocked ? `Add sessions — ${enrollLockedName}` : "Enroll child"}
 onClose={() => setOpenEnroll(false)}
 >
 <div className="muted">
 If the child has an existing balance, you can choose “Use existing balance”.
 </div>
...
/>
 ...
 <div className="muted">
 Existing sessions balance: <b>{Number(pkgInfo.sessions_remaining || 0)}</b>{" "}
 — Remaining to pay:{" "}
 ...
 <div className="muted" style={{ marginTop: 8 }}>
 Upcoming sessions in this run: <b>{singlePreview.runFuture}</b> — Allocated now: <b>{singlePreview.allocNow}</b> — Carry:{" "}
...
/>
 ...
 onChange={(e) => setNewChildForm((p) => ({ ...p, birth_date: e.target.value }))}
 />
 </div>
 ...
 onChange={(e) => setNewChildForm((p) => ({ ...p, class: e.target.value }))}
 placeholder="e.g. A1"
 />
 </div>
 ...
 onChange={(e) => setNewChildForm((p) => ({ ...p, gender: e.target.value }))}
 >
 ...
 onChange={(e) => setNewChildForm((p) => ({ ...p, mother_phone: e.target.value }))}
 placeholder=""
 />
 </div>
 ...
 onChange={(e) => setNewChildForm((p) => ({ ...p, father_phone: e.target.value }))}
 />
 </div>
 ...
 onChange={(e) => setNewChildForm((p) => ({ ...p, notes: e.target.value }))}
 />
 </div>
 ...
 disabled={newChildSaving}
 >
 {newChildSaving ? "Saving…" : newChildEnrollNow ? "Create & enroll" : "Create"}
 </button>
 </div>
 </div>
 </Modal>
