...
function getPresetRange(key) {
  ...
  switch (key) {
    case "this_month":
      return { from: som(y, m), to: eom(y, m) };
    case "last_month": {
      const lm = m === 0 ? 11 : m - 1,
        ly = m === 0 ? y - 1 : y;
      return { from: som(ly, lm), to: eom(ly, lm) };
    }
    case "last_3m": {
      const s = new Date(y, m - 2, 1);
      return { from: som(s.getFullYear(), s.getMonth()), to: eom(y, m) };
    }
    case "last_6m": {
...
function SectionHeader({ icon: Icon, children, color }) {
  return (
    <div
      className="an-section-header"
      style={
        color
          ? { borderImage: `linear-gradient(180deg, ${color}, ${color}33) 1` }
          : undefined
      }
    >
      <h2>{children}</h2>
...
function DonutChart({
  ...
  function buildArc(ro, ri, sA, sweep) {
    if (sweep >= 2 * Math.PI - 0.001) {
      ...
      const x2o = cx + ro * Math.cos(midA),
        y2o = cy + ro * Math.sin(midA);
      const x1i = cx + ri * Math.cos(sA),
        y1i = cy + ri * Math.sin(sA);
      const x2i = cx + ri * Math.cos(midA),
        y2i = cy + ri * Math.sin(midA);
      return (
        `M${x1o} ${y1o} A${ro} ${ro} 0 1 1 ${x2o} ${y2o} A${ro} ${ro} 0 1 1 ${x1o} ${y1o} ` +
    ...
    const x1 = cx + ro * Math.cos(sA),
      y1 = cy + ro * Math.sin(sA);
    const x2 = cx + ro * Math.cos(eA),
      y2 = cy + ro * Math.sin(eA);
    const ix1 = cx + ri * Math.cos(sA),
      iy1 = cy + ri * Math.sin(sA);
    const ix2 = cx + ri * Math.cos(eA),
      iy2 = cy + ri * Math.sin(eA);
  ...
  const centerVal = hovData
    ? fmtMoneyShort(hovData.value)
    : fmtMoneyShort(total);
    ...
    >
      {slices.map((s, i) => (
        <path
          ...
          fill={s.color}
          opacity={
            !mounted ? 0 : hoverSlice === null || hoverSlice === i ? 0.92 : 0.45
          }
            ...
            {data.map((item, i) => (
              <div
                ...
                style={{
                  ...
                  cursor: "default",
                  background:
                    donutHoverIdx === i ? `${item.color}18` : "transparent",
                }}
              >
                {/* Colored dot */}
                <div
          ...
          <>
            ...
            >
              <span
                ...
                style={{ background: data[donutHoverIdx].color }}
              />
              <span className="an-tooltip-title" style={{ margin: 0 }}>
                {data[donutHoverIdx].label}
              </span>
            </div>
            <div className="an-tooltip-row">
              <span className="an-tooltip-label">المبلغ</span>
              <span className="an-tooltip-val">
                {fmtMoney(data[donutHoverIdx].value)} ₪
              </span>
            </div>
            <div className="an-tooltip-row">
          ...
          <>
            ...
            {hov.expense > 0 && (
              <div className="an-tooltip-row">
                ...
                <span className="an-tooltip-val">
                  {fmtMoney(hov.expense)} ₪
                </span>
              </div>
            )}
            {hov.income > 0 && (
              <div
                className="an-tooltip-row"
                style={{
                  marginTop: 4,
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontWeight: 800,
                    color:
                      hov.income - hov.expense >= 0 ? "#4ade80" : "#f87171",
                  }}
                >
          ...
          return (
            <g key={gi}>
              <line
                ...
                x2={W - pad.right}
                y2={gy}
                stroke={gi === 4 ? "#e2e8f0" : "#f8fafc"}
                strokeWidth={gi === 4 ? 1.5 : 1}
              />
              <text
                x={pad.left - 6}
                y={gy}
            ...
            <g key={i}>
              ...
              <rect
                ...
                style={{
                  ...
                  transition:
                    "height .55s cubic-bezier(.4,0,.2,1), y .55s cubic-bezier(.4,0,.2,1), opacity .15s",
                  filter: isHov
                    ? `drop-shadow(0 4px 10px ${accentColor}66)`
                    : "none",
                }}
              />
        ...
        {hoverIdx !== null && visible[hoverIdx] && (
          <>
            ...
            <div className="an-tooltip-row">
              ...
              <span className="an-tooltip-val">
                {fmtMoney(visible[hoverIdx].value)} ₪
              </span>
            </div>
            <div className="an-tooltip-row">
              <span className="an-tooltip-label">النسبة</span>
    ...
    >
      {data.slice(0, 10).map((item, i) => (
        ...
        >
          ...
          <span
            style={{ flex: 1, fontSize: 13, fontWeight: 700, color: "#334155" }}
          >
            {item.label || "غير محدد"}
          </span>
...
export default function Analytics() {
  ...
  const _s = (() => {
    try {
      return JSON.parse(localStorage.getItem("pgx_analytics_v1")) || {};
    } catch {
      return {};
    }
  })();
  ...
  useEffect(() => {
    function handler(e) {
      ...
      if (kindDropRef.current && !kindDropRef.current.contains(e.target))
        setKindDropOpen(false);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "pgx_analytics_v1",
        JSON.stringify({ preset, customFrom, customTo, selCourses, selKinds }),
      );
    } catch {}
    ...
    try {
      const [
        ...
        supabase
          ...
          .select(
            "amount, created_at, child_id, course_id, course_title, run_id",
          ),
        supabase
          .from("expenses")
      ...
      const cmap = {};
      for (const c of kids || []) {
        if (c.id) cmap[c.id] = c.country || "غير محدد";
      }
      setChildMap(cmap);
      const kmap = {};
      for (const c of courses || []) {
        if (c.id) kmap[String(c.id)] = c.kind || "course";
      }
  ...
  const runCourseMap = useMemo(() => {
    ...
    for (const p of payments) {
      if (p.run_id && p.course_id) map[p.run_id] = String(p.course_id);
    }
    return map;
  }, [payments]);
  ...
  const courseOptions = useMemo(() => {
    ...
    for (const p of payments) {
      if (p.course_id && p.course_title)
        map[String(p.course_id)] = p.course_title;
    }
  ...
  const selectedPresetLabel =
    PRESETS.find((p) => p.key === preset)?.label ?? "الفترة";
  ...
  const isKindMatch = (courseId, kinds) => {
    ...
    return kinds.some((sk) =>
      sk === "workshop" ? k === "workshop" : k !== "workshop",
    );
  };
  ...
  const fPays = useMemo(() => {
    ...
    if (from && to)
      d = d.filter((p) => {
        const x = p.created_at?.slice(0, 10);
        return x && x >= from && x <= to;
      });
    if (selCourses.length > 0)
      d = d.filter((p) => selCourses.includes(String(p.course_id)));
    if (selKinds.length > 0)
      d = d.filter((p) => isKindMatch(p.course_id, selKinds));
    return d;
  ...
  const fExps = useMemo(() => {
    let d = expenses;
    if (from && to)
      d = d.filter((e) => e.spent_on && e.spent_on >= from && e.spent_on <= to);
    if (selCourses.length > 0)
      d = d.filter(
        (e) => e.run_id && selCourses.includes(String(runCourseMap[e.run_id])),
      );
    if (selKinds.length > 0)
      d = d.filter((e) => {
  ...
  const kindComparison = useMemo(() => {
    const calc = (kindCheck) => {
      const pays = fPays.filter((p) =>
        kindCheck(courseKindMap[String(p.course_id)] || "course"),
      );
      ...
      const students = new Set(pays.map((p) => p.child_id).filter(Boolean))
        .size;
  ...
  const fStaff = useMemo(() => {
    ...
    return staffPayments.filter(
      (s) => s.date_to && s.date_to >= from && s.date_to <= to,
    );
  ...
  const kpi = useMemo(() => {
    ...
    const staffExpIds = new Set(
      staffPayments.map((s) => s.expense_id).filter(Boolean),
    );
    const nonStaffExp = fExps
      .filter((e) => !staffExpIds.has(e.id))
      .reduce((s, e) => s + Number(e.amount || 0), 0);
    const staffCost = fStaff.reduce(
    ...
    const activeStudents = new Set(fPays.map((p) => p.child_id).filter(Boolean))
      .size;
    return {
      totalIncome,
      totalExpenses,
  ...
  const monthlyTrend = useMemo(() => {
    ...
    for (const p of fPays) {
      ...
      if (mo) inc[mo] = (inc[mo] || 0) + Number(p.amount || 0);
    }
    for (const e of fExps) {
      const mo = e.spent_on?.slice(0, 7);
      if (mo) exp[mo] = (exp[mo] || 0) + Number(e.amount || 0);
    }
  ...
  const expByCategory = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([label, value], i) => ({
        label,
        value,
        color: RED_PALETTE[i % RED_PALETTE.length],
        pct: total ? ((value / total) * 100).toFixed(1) : 0,
      }))
      .sort((a, b) => b.value - a.value);
  }, [fExps]);
  ...
  const topItems = useMemo(() => {
    const map = {};
    for (const e of fExps) {
      if (e.item) map[e.item] = (map[e.item] || 0) + Number(e.amount || 0);
    }
    const total = Object.values(map).reduce((s, v) => s + v, 0);
    return Object.entries(map)
      .map(([label, value]) => ({
  ...
  const coursePL = useMemo(() => {
    const runCourse = {};
    for (const p of payments) {
      if (p.run_id && p.course_id) runCourse[p.run_id] = p.course_id;
    }
    const courseNames = {},
      incMap = {},
      expMap = {};
  ...
  const sortedCoursePL = useMemo(() => {
    return [...coursePL].sort((a, b) =>
      plSort.dir === "desc"
        ? b[plSort.col] - a[plSort.col]
        : a[plSort.col] - b[plSort.col],
    );
  }, [coursePL, plSort]);
  ...
  return (
    <div
      className="container an-page page page--analytics"
      style={{ direction: "rtl" }}
    >
      <PageHeader
        title={
        ...
        <div className="an-sidebar">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              >
                ...
                <ChevronDown
                  ...
                  style={{
                    ...
                    transform: dateDropOpen ? "rotate(180deg)" : "rotate(0deg)",
                  }}
                />
              </button>
            ...
            {from && to && preset !== "all" && preset !== "custom" && (
              <span
                style={{ fontSize: 11.5, color: "#94a3b8", fontWeight: 700 }}
              >
          ...
          <div
            className="an-filter-divider"
            style={{ height: "auto", alignSelf: "stretch", margin: "0 4px" }}
          />
            ...
            <div className="an-dropdown" ref={kindDropRef}>
              ...
              >
                ...
                <ChevronDown
                  ...
                  style={{
                    transition: "transform .2s",
                    transform: kindDropOpen ? "rotate(180deg)" : "rotate(0deg)",
                  }}
                />
              </button>
              {kindDropOpen && (
        ...
        <div className="an-main">
          ...
          {loading && (
            <>
                ...
                {[1, 2, 3, 4, 5].map((i) => (
                  <div key={i} className="an-card" style={{ minHeight: 108 }}>
                    <div
                      ...
                      style={{ height: 11, width: "50%", marginBottom: 14 }}
                    />
                    <div
                      className="an-skeleton"
                      style={{ height: 34, width: "72%", marginBottom: 10 }}
                    />
                    <div
                      className="an-skeleton"
                      style={{ height: 9, width: "38%" }}
                    />
                  </div>
                ))}
              </div>
              <div className="an-card" style={{ marginBottom: 36 }}>
                <div
                  className="an-skeleton"
                  style={{ height: 14, width: "28%", marginBottom: 22 }}
                />
                <div
                  className="an-skeleton"
          ...
          {!loading && (
            <>
                  ...
                  <>
                    ...
                    >
                      <KpiCard
                        icon={TrendingUp}
                        label="إجمالي الدخل"
                        value={<Shekel n={kpi.totalIncome} />}
                        hint="المبالغ المدفوعة"
                        variant="ok"
                      />
                      <KpiCard
                        icon={TrendingDown}
                        label="المصاريف التشغيلية"
                        value={<Shekel n={kpi.nonStaffExp} />}
                        hint="بدون رواتب الموظفين"
                        variant="danger"
                      />
                      <KpiCard
                        icon={Briefcase}
                        label="تكاليف الموظفين"
                        value={<Shekel n={kpi.staffCost} />}
                        hint="رواتب مدفوعة"
                        variant="info"
                      />
                      <KpiCard
              ...
              {/* 1b · Kind comparison */}
              {(kindComparison.courses.income > 0 ||
                kindComparison.workshops.income > 0) && (
                      ...
                      {[
                        {
                          label: "دورات",
                          color: "#3b82f6",
                          data: kindComparison.courses,
                        },
                        {
                          label: "ورشات",
                          color: "#8b5cf6",
                          data: kindComparison.workshops,
                        },
                      ].map(({ label, color, data }) => {
                        const totalIncome =
                          kindComparison.courses.income +
                            kindComparison.workshops.income || 1;
                        const sharePct = (
                          ...
                          >
                            ...
                            {[
                              ...
                              },
                              {
                                label: "صافي الربح",
                                val: data.net,
                                color: data.net >= 0 ? "#00ac47" : "#ef4444",
                              },
                            ].map((row) => (
                              <div
                                key={row.label}
                              ...
                              >
                                ...
                                <span
                                  style={{
                                    fontSize: 13,
                                    fontWeight: 900,
                                    color: data.margin >= 0 ? color : "#ef4444",
                                  }}
                                >
              ...
              <div className="an-section">
                ...
                <div className="an-2col">
                  <Card>
                    <CardTitle
                      action={
                        incomeByCourse.length > 0
                          ? `${incomeByCourse.length} دورة`
                          : undefined
                      }
                    >
                      الدخل حسب الدورة
              ...
              {/* 4 · Expense breakdown */}
              <div className="an-section">
                <SectionHeader icon={ShoppingCart} color="#ef4444">
                  تحليل المصاريف
                </SectionHeader>
                <div className="an-2col" style={{ marginBottom: 20 }}>
                  <ExpenseCategoryCard data={expByCategory} />
                  <Card>
                    <CardTitle action="أعلى 10">
                      أكثر الأشخاص والمتاجر إنفاقاً
                    </CardTitle>
                      ...
                      <table className="an-pl-table">
                        <thead>
                          <tr>
                            ...
                            <th
                              className={
                                plSort.col === "income" ? "an-sort-active" : ""
                              }
                            ...
                            <th
                              className={
                                plSort.col === "expense" ? "an-sort-active" : ""
                              }
                        ...
                        <tbody>
                          {sortedCoursePL.map((c) => {
                            const margin =
                              c.income > 0 ? (c.net / c.income) * 100 : 0;
                            const isPos = c.net >= 0;
                            return (
                              <tr key={c.id}>
                                <td>{c.name}</td>
                                <td
                                  style={{ color: "#00ac47", fontWeight: 800 }}
                                >
                                  {fmtMoney(c.income)} ₪
                                </td>
                                <td
                                ...
                                <td>
                                  <span
                                    className={`an-net-badge ${isPos ? "an-pos" : "an-neg"}`}
                                    style={{
                                      background: isPos
                                        ? "rgba(0,172,71,0.08)"
                                        : "rgba(239,68,68,0.08)",
                    ...
                    {kpi.totalIncome > 0 ? (
                      (() => {
                        const staffPct = Math.min(
                          100,
                          (kpi.staffCost / kpi.totalIncome) * 100,
                        );
                        const expPct = Math.min(
                          100 - staffPct,
                          (kpi.nonStaffExp / kpi.totalIncome) * 100,
                        );
